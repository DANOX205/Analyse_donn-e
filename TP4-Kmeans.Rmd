...
fviz_pca_ind(resacp,geom = c("point"),  repel = TRUE)
fviz_pca_var(resacp)
```
...
```{r,eval=F}
# Il faut centrer/réduire les variables.
wineCR <- scale(wine[,-c(1,2)], center=TRUE,scale=TRUE)
...
```{r,eval=F}
help(kmeans)
reskmeans<-kmeans(wineCR,centers = 3)
```
...
```{r,eval=F}
# Pas Compris comment utiliser la fonction adjustedRandIndex et je ne sais pas quelle fonction de la librairie mclust il faut utiliser.
table(wine$Qualite,wine$Type)
adjustedRandIndex(wine$Qualite,wine$Type)

```

```{r, eval=F}
clust<-paste("Cl-K",reskmeans$cluster,sep="")
Tab<-melt(table(clust,wine[,1])) # On peut changer le 1 par un 2 pour avoir la corrélation entre le type de vin et les 3 Clusters
...
Iintra<-NULL
for (k in 2:Kmax){
  resaux<-kmeans(wineCR,centers=k)
...
aux<-silhouette(reskmeanscl[,3], daisy(wineCR))
fviz_silhouette(aux)+
...
resPAM<-pam(wineCR,k=3,metric=c("euclidean"))
resPAM$medoids
resPAM$id.med
...
adjustedRandIndex(resPAMcl[,4],wine$Type)
table(wine$Type,resPAMcl[,4])
adjustedRandIndex(resPAMcl[,4],wine$Qualite)
table(wine$Qualite,resPAMcl[,4])
```
